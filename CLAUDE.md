...
- Pin to a specific `Version` header value and test before upgrading
- Handle rate limits gracefully with exponential backoff
- Store API keys and secrets in environment variables, never in code
- Verify webhook signatures on incoming webhook requests
- Implement idempotency when processing webhooks (deduplicate by event ID)

### GHL Customization Workflow
1. **White-label first** — Set up branding, domains, logos before building pages
2. **Snapshot-based onboarding** — Create a master snapshot with all branded templates; use builder settings for visual consistency (survives CSS breakage)
3. **Centralized CSS** — Host brand stylesheet on CDN, link in all funnels/websites
4. **Component library** — Build reusable global sections: Header/Nav, Hero, Feature Grid, Testimonials, CTA Banner, Contact Form, FAQ, Pricing Table, Footer
5. **Test responsively** — Always preview desktop, tablet, and mobile; fix with custom CSS media queries; manually set mobile font sizes
6. **Version CSS** — Use versioned filenames for cache-busting
7. **Accessibility audit** — Test with axe DevTools, check contrast ratios, verify keyboard navigation, review form labels
8. **Audit after GHL updates** — Platform updates can break custom CSS (class name changes)

### Design Principles for Compass CRM
- **Brand consistency** across all touchpoints — funnels, forms, emails, portal, chat
- **Trust and credibility** — CRM tools handle business data; design should convey security and professionalism with clean layouts and appropriate visual weight
- **Mobile-first** — Most end-user interactions happen on mobile
- **Visual hierarchy** — CRM users are task-oriented; interfaces should have clear hierarchy, scannable layouts, and obvious CTAs
...
- **Performance** — Keep custom JS minimal; lazy-load external resources; optimize images (WebP, < 200KB); GHL pages can be heavy
